...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('lol')
        .setDescription('League of Legends lookup tool.')
        .addSubcommand(subcommand =>
            subcommand
                .setName('freerotation')
                .setDescription('Current Weekly Free Rotation Champions'))
        .addSubcommand(subcommand =>
            subcommand
                .setName('mastery')
                .setDescription('Shows a player\'s highest mastery champions.')
                .addStringOption(option => option.setName('summoner').setDescription("SummonerName#Tag").setRequired(true)))
        .addSubcommand(subcommand =>
            subcommand
                .setName('rank')
                .setDescription('Rank information of a player.')
                .addStringOption(option => option.setName('summoner').setDescription("SummonerName#Tag").setRequired(true)))
        .addSubcommand(subcommand =>
            subcommand
                .setName('match')
                .setDescription('Match history of a player.')
                .addStringOption(option => option.setName('summoner').setDescription("SummonerName#Tag").setRequired(true))),
        async execute(interaction, client) { 
            ...
            // Extract the lol subcommand
            const subcommand = interaction.options._subcommand;
            const options = interaction.options._hoistedOptions[0];

            if (subcommand != '__match__' && subcommand != 'freerotation') {      
                // Extract summoner name and tag line
                summonerName = utf8.encode(options.value);
                ...
                console.log("name#tag: ", summonerName, tagLine);
                ...
                if (riotIDResponse.hasOwnProperty('status') && riotIDResponse.status.status_code == 404) {
                    await interaction.editReply("This summoner name cannot be found!");
                    return;
                }
            ...
            if (subcommand == 'mastery') {
            ...
            } else if (subcommand == 'rank') { // Player Rank Lookup (Solo and Flex)
            ...
            } else if (subcommand == 'freerotation') {
                freeChamps = [];
                freeChampsStr = '';
                freeChampsForNew = [];
                freeChampsForNewStr = '';
                ...
                await interaction.editReply({ embeds: [freeRotationEmbed] });
